...
def SEResNetBottleneck(filters, reduction=16, strides=1, **kwargs):
    ...
    def layer(input_tensor):
        ...
        # bottleneck
        x = tf.keras.layers.Conv2D(filters // 4, (1, 1), kernel_initializer='he_uniform',
        ...
        x = tf.keras.layers.Conv2D(filters // 4, (3, 3),
                          kernel_initializer='he_uniform', use_bias=False)(x)
        x = tf.keras.layers.BatchNormalization(**bn_params)(x)
        x = tf.keras.layers.Activation('relu')(x)

        x = tf.keras.layers.Conv2D(filters, (1, 1), kernel_initializer='he_uniform', use_bias=False)(x)
        x = tf.keras.layers.BatchNormalization(**bn_params)(x)
        ...
        if strides != 1 or x_channels != r_channels:
            residual = tf.keras.layers.Conv2D(x_channels, (1, 1), strides=strides,
                                     kernel_initializer='he_uniform', use_bias=False)(residual)
            residual = tf.keras.layers.BatchNormalization(**bn_params)(residual)
...
def SEResNeXtBottleneck(filters, reduction=16, strides=1, groups=32, base_width=4, **kwargs):
    ...
    def layer(input_tensor):
        ...
        # bottleneck
        x = tf.keras.layers.Conv2D(width, (1, 1), kernel_initializer='he_uniform', use_bias=False)(x)
        x = tf.keras.layers.BatchNormalization(**bn_params)(x)
        x = tf.keras.layers.Activation('relu')(x)
...
def SEBottleneck(filters, reduction=16, strides=1, groups=64, is_first=False, **kwargs):
    ...
    def layer(input_tensor):
        ...
        # bottleneck
        x = tf.keras.layers.Conv2D(filters // 2, (1, 1), kernel_initializer='he_uniform', use_bias=False)(x)
        x = tf.keras.layers.BatchNormalization(**bn_params)(x)
        x = tf.keras.layers.Activation('relu')(x)
        ...
        if strides != 1 or x_channels != r_channels:
            if padding:
                residual = tf.keras.layers.ZeroPadding2D(1)(residual)
            residual = tf.keras.layers.Conv2D(x_channels, downsample_kernel_size, strides=strides,
                                     kernel_initializer='he_uniform', use_bias=False)(residual)
            residual = tf.keras.layers.BatchNormalization(**bn_params)(residual)
...
def SENet(
    ...
    if input_tensor is None:
        ...
        if not backend.is_keras_tensor(input_tensor):
            input = tf.keras.Input(tensor=input_tensor, shape=input_shape)
        else:
            input = input_tensor
    ...
    if model_params.input_3x3:
        ...
        x = tf.keras.layers.ZeroPadding2D(1)(x)
        x = tf.keras.layers.Conv2D(init_filters, (3, 3), strides=2,
                          use_bias=False, kernel_initializer='he_uniform')(x)
        x = tf.keras.layers.BatchNormalization(**bn_params)(x)
        x = tf.keras.layers.Activation('relu')(x)

        x = tf.keras.layers.ZeroPadding2D(1)(x)
        x = tf.keras.layers.Conv2D(init_filters, (3, 3), use_bias=False,
                          kernel_initializer='he_uniform')(x)
        x = tf.keras.layers.BatchNormalization(**bn_params)(x)
        x = tf.keras.layers.Activation('relu')(x)

        x = tf.keras.layers.ZeroPadding2D(1)(x)
        x = tf.keras.layers.Conv2D(init_filters * 2, (3, 3), use_bias=False,
                          kernel_initializer='he_uniform')(x)
        x = tf.keras.layers.BatchNormalization(**bn_params)(x)
        x = tf.keras.layers.Activation('relu')(x)
        ...
        x = tf.keras.layers.Conv2D(init_filters, (7, 7), strides=2, use_bias=False,
                          kernel_initializer='he_uniform')(x)
        x = tf.keras.layers.BatchNormalization(**bn_params)(x)
        x = tf.keras.layers.Activation('relu')(x)
