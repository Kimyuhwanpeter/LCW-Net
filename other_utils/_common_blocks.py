...
def GroupConv2D(filters,
    ...
    def layer(input_tensor):
        inp_ch = int(tf.keras.backend.int_shape(input_tensor)[-1] // groups)  # input grouped channels
...
def ChannelSE(reduction=16, **kwargs):
    ...
    def layer(input_tensor):
        ...
        x = tf.keras.layers.Lambda(expand_dims, arguments={'channels_axis': channels_axis})(x)
        x = tf.keras.layers.Conv2D(channels // reduction, (1, 1), kernel_initializer='he_uniform')(x)
        x = tf.keras.layers.Activation('relu')(x)
        x = tf.keras.layers.Conv2D(channels, (1, 1), kernel_initializer='he_uniform')(x)
