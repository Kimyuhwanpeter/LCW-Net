# -*- coding:utf-8 -*-
import other_utils.resnet as resnet
...
irv2 = tf.keras.applications.inception_resnet_v2
iv3 = tf.keras.applications.inception_v3
...
class BackbonesFactory(ModelsFactory):
    ...
    def get_backbone(self, name, *args, **kwargs):
        ...
        if name == "resnet18":
            model_fn = resnet.ResNet18
        if name == "resnet34":
            model_fn = resnet.ResNet34
        if name == "resnet50":
            model_fn = resnet.ResNet50
        if name == "resnet101":
            model_fn = resnet.ResNet101
        if name == "resnet152":
            model_fn = resnet.ResNet152
        if name == "resnext50":
            model_fn = resnext.ResNeXt50
        if name == "resnext101":
            model_fn = resnext.ResNeXt101
        if name == "mobilenetv2":
